...
#analogous file to main.py, but uses ST data
from_week = 1 #start week no. of analysis
weeks_to_analyze = 70 #max of 70 weeks in database

results =  pd.DataFrame()
benchResults = pd.DataFrame()
ncavPortfolio = pd.DataFrame()

ncavPortfolio = pd.read_csv('data/stocks_ST/AA.csv', usecols=["Date"])
...
#benchmark performance block
    if filename == 'SPY.csv':
        combo = pd.read_csv(r'data/stocks_ST/' + filename, usecols=["Date", "Adj Close"])
        ...
        combo.to_csv('./benchmarkST.csv', index=False)
...
results.to_csv('./aggregateST.csv', index=False)
...
ncavPortfolio.to_csv('./printoutST.csv', index=False)